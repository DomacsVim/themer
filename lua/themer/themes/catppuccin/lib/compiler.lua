local path_sep = require("themer.themes.catppuccin").path_sep
...
function M.compile(flavour)
	local theme = require("themer.themes.catppuccin.lib.mapper").apply(flavour)
	local lines = {
		[[
require("themer.themes.catppuccin").compiled = string.dump(function()
...
Below is the error message that we captured:
	...
	file:write(require("themer.themes.catppuccin").compiled)
	file:close()
end
